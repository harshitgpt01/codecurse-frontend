...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --bg:#03020a; --surface:#0a0914; --card:#0d0c1a; --card2:#110f1f;
    --border:rgba(255,255,255,.07); --red:#ff2d2d; --orange:#ff6a1a;
    --purple:#7c3aed; --text:#ede8ff; --muted:#5a5472; --dim:#2a2840;
    --glow:rgba(255,45,45,.25); --mono:'JetBrains Mono',monospace; --display:'Syne',sans-serif;
  }
  .hp-root { min-height:100vh; background:var(--bg); color:var(--text); font-family:var(--mono); cursor:auto; position:relative; }
  .hp-root::before { content:''; position:fixed; inset:0; z-index:0; pointer-events:none; opacity:.025; background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='.85' numOctaves='4'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E"); background-size:180px; }

  .hp-nav { position:sticky; top:0; z-index:100; display:flex; align-items:center; justify-content:space-between; padding:0 36px; height:64px; background:rgba(3,2,10,.92); border-bottom:1px solid var(--border); backdrop-filter:blur(24px); animation:hp-nav-in .6s cubic-bezier(.16,1,.3,1) both; }
  ...
  .hp-logo-skull { width:36px; height:36px; border-radius:9px; background:linear-gradient(135deg,var(--red),var(--orange)); display:flex; align-items:center; justify-content:center; font-size:17px; position:relative; overflow:hidden; flex-shrink:0; box-shadow:0 0 18px var(--glow); animation:hp-pulse 3s ease-in-out infinite; }
  @keyframes hp-pulse { 0%,100%{box-shadow:0 0 18px var(--glow)} 50%{box-shadow:0 0 32px rgba(255,45,45,.45)} }
  .hp-logo-skull::after { content:''; position:absolute; inset:0; background:linear-gradient(135deg,transparent 40%,rgba(255,255,255,.22)); animation:hp-shimmer 3s linear infinite; }
  ...
  .hp-nav-center { display:flex; align-items:center; gap:4px; background:rgba(255,255,255,.03); border:1px solid var(--border); border-radius:10px; padding:4px; }
  .hp-nav-tab { padding:6px 16px; border-radius:7px; font-family:var(--mono); font-size:.72rem; color:var(--muted); background:none; border:none; cursor:pointer; transition:all .2s; text-decoration:none; display:flex; align-items:center; gap:6px; }
  .hp-nav-tab:hover { color:var(--text); background:rgba(255,255,255,.05); }
  .hp-nav-tab.active { background:linear-gradient(135deg,rgba(255,45,45,.15),rgba(255,106,26,.1)); color:var(--orange); border:1px solid rgba(255,45,45,.15); }
  .hp-nav-right { display:flex; align-items:center; gap:10px; position:relative; }
  .hp-streak { display:flex; align-items:center; gap:6px; padding:6px 12px; border-radius:8px; background:rgba(251,191,36,.06); border:1px solid rgba(251,191,36,.18); font-size:.7rem; color:#fbbf24; transition:all .2s; }
  .hp-streak:hover { background:rgba(251,191,36,.1); }
  .hp-user-btn { display:flex; align-items:center; gap:8px; padding:5px 12px 5px 5px; border-radius:10px; background:rgba(255,255,255,.04); border:1px solid var(--border); cursor:pointer; font-family:var(--mono); font-size:.78rem; color:var(--text); transition:all .2s; }
  .hp-user-btn:hover { border-color:rgba(255,45,45,.3); background:rgba(255,45,45,.05); }
  .hp-avatar { width:30px; height:30px; border-radius:8px; background:linear-gradient(135deg,var(--red),var(--orange)); display:flex; align-items:center; justify-content:center; font-size:.72rem; font-weight:700; color:#fff; flex-shrink:0; box-shadow:0 0 10px var(--glow); }
  .hp-dropdown { position:absolute; top:calc(100% + 10px); right:0; background:#0e0d1c; border:1px solid rgba(255,255,255,.1); border-radius:14px; min-width:210px; box-shadow:0 24px 60px rgba(0,0,0,.8),0 0 0 1px rgba(255,45,45,.08); overflow:hidden; z-index:200; animation:hp-dropIn .2s cubic-bezier(.16,1,.3,1) both; }
  @keyframes hp-dropIn { from{opacity:0;transform:translateY(-10px) scale(.96)} to{opacity:1;transform:none} }
  .hp-dropdown-header { padding:14px 16px 12px; border-bottom:1px solid var(--border); background:linear-gradient(135deg,rgba(255,45,45,.05),transparent); }
  ...
  .hp-dropdown-item { display:flex; align-items:center; gap:10px; padding:11px 16px; font-size:.78rem; color:var(--muted); text-decoration:none; cursor:pointer; transition:all .2s; background:none; border:none; width:100%; text-align:left; font-family:var(--mono); }
  ...
  .hp-hero { position:relative; overflow:hidden; padding:36px 36px 32px; border-bottom:1px solid var(--border); }
  .hp-hero::before { content:''; position:absolute; inset:0; background-image:linear-gradient(rgba(255,45,45,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(255,45,45,.04) 1px,transparent 1px); background-size:36px 36px; animation:hp-grid 24s linear infinite; mask-image:radial-gradient(ellipse 80% 100% at 20% 50%,black,transparent); }
  @keyframes hp-grid { from{background-position:0 0} to{background-position:36px 36px} }
  .hp-hero-orb1 { position:absolute; width:320px; height:320px; border-radius:50%; background:radial-gradient(circle,rgba(255,45,45,.14),transparent 70%); filter:blur(70px); top:-80px; left:-60px; pointer-events:none; animation:hp-orb 8s ease-in-out infinite; }
  .hp-hero-orb2 { position:absolute; width:200px; height:200px; border-radius:50%; background:radial-gradient(circle,rgba(124,58,237,.12),transparent 70%); filter:blur(60px); bottom:-40px; right:80px; pointer-events:none; animation:hp-orb 10s ease-in-out -3s infinite; }
  ...
  .hp-hero-eyebrow { font-size:.65rem; color:var(--red); letter-spacing:3px; text-transform:uppercase; margin-bottom:8px; display:flex; align-items:center; gap:8px; }
  .hp-hero-eyebrow::before { content:''; display:inline-block; width:20px; height:1px; background:var(--red); }
  .hp-hero-title { font-family:var(--display); font-size:clamp(1.4rem,3vw,2rem); font-weight:800; letter-spacing:-1px; line-height:1.1; }
  .hp-hero-title em { color:var(--red); font-style:normal; background:linear-gradient(90deg,var(--red),var(--orange)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
  ...
  .hp-stat-card { background:var(--card); border:1px solid var(--border); border-radius:14px; padding:18px 16px; display:flex; align-items:center; gap:14px; transition:all .25s cubic-bezier(.16,1,.3,1); position:relative; overflow:hidden; }
  .hp-stat-card::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; opacity:0; transition:opacity .25s; }
  ...
  .hp-stat-icon { width:42px; height:42px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1.1rem; flex-shrink:0; }
  ...
  .hp-section-title { font-family:var(--display); font-size:1.05rem; font-weight:800; letter-spacing:-.3px; display:flex; align-items:center; gap:8px; }
  ...
  .hp-diff-tab { padding:5px 14px; border-radius:7px; font-size:.68rem; border:1px solid var(--border); background:none; color:var(--muted); cursor:pointer; transition:all .2s; font-family:var(--mono); }
  ...
  .hp-search { width:100%; padding:10px 14px 10px 38px; background:var(--card); border:1px solid var(--border); border-radius:10px; color:var(--text); font-family:var(--mono); font-size:.78rem; outline:none; transition:all .2s; caret-color:var(--red); }
  ...
  .hp-select { padding:10px 32px 10px 14px; background:var(--card); border:1px solid var(--border); border-radius:10px; color:var(--text); font-family:var(--mono); font-size:.75rem; outline:none; cursor:pointer; transition:all .2s; appearance:none; }
  ...
  .hp-clear-btn { padding:10px 14px; border-radius:10px; font-size:.72rem; background:rgba(255,45,45,.07); border:1px solid rgba(255,45,45,.18); color:#ff8a50; white-space:nowrap; cursor:pointer; transition:all .2s; font-family:var(--mono); }
  .hp-clear-btn:hover { background:rgba(255,45,45,.12); }

  .hp-table-head { display:grid; grid-template-columns:52px 1fr 120px 110px 90px; padding:10px 20px; margin-bottom:8px; font-size:.62rem; color:#8b87a8; letter-spacing:2.5px; text-transform:uppercase; background:rgba(255,255,255,.025); border:1px solid rgba(255,255,255,.06); border-radius:10px; }
  .hp-problem-row { display:grid; grid-template-columns:52px 1fr 120px 110px 90px; align-items:center; background:var(--card); border:1px solid var(--border); border-radius:12px; padding:15px 20px; margin-bottom:7px; text-decoration:none; color:var(--text); position:relative; overflow:hidden; transition:all .22s cubic-bezier(.16,1,.3,1); }
  .hp-problem-row::before { content:''; position:absolute; inset:0; background:linear-gradient(90deg,transparent,rgba(255,45,45,.03),transparent); transform:translateX(-100%); transition:transform .5s; }
  .hp-problem-row::after { content:''; position:absolute; left:0; top:8px; bottom:8px; width:3px; background:linear-gradient(180deg,var(--red),var(--orange)); border-radius:0 3px 3px 0; transform:scaleY(0); transform-origin:center; transition:transform .22s cubic-bezier(.16,1,.3,1); }
  .hp-problem-row:hover { border-color:rgba(255,45,45,.2); background:var(--card2); transform:translateX(4px); box-shadow:0 4px 28px rgba(0,0,0,.5),-4px 0 24px rgba(255,45,45,.08); }
  ...
  .hp-row-title { font-size:.86rem; font-weight:600; color:var(--text); transition:color .2s; padding-right:12px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
  .hp-problem-row:hover .hp-row-title { color:#ff8a50; }

  .hp-badge { display:inline-flex; align-items:center; justify-content:center; padding:4px 11px; border-radius:6px; font-size:.63rem; font-weight:700; letter-spacing:.5px; text-transform:capitalize; }
  ...
  .hp-solved-pill { display:inline-flex; align-items:center; gap:5px; padding:4px 10px; border-radius:6px; font-size:.63rem; font-weight:600; background:rgba(74,222,128,.08); color:#4ade80; border:1px solid rgba(74,222,128,.18); }
  .hp-unsolved-dash { font-size:.7rem; color:var(--dim); }

  .hp-empty { text-align:center; padding:70px 20px; display:flex; flex-direction:column; align-items:center; gap:14px; }
  .hp-empty-icon { font-size:3rem; opacity:.2; display:block; }
  .hp-empty-text { font-size:.82rem; color:var(--muted); }
  .hp-empty-btn { padding:9px 20px; background:rgba(255,45,45,.08); border:1px solid rgba(255,45,45,.2); border-radius:9px; color:#ff8a50; font-family:var(--mono); font-size:.75rem; cursor:pointer; transition:all .2s; }
      ...
      <div className="hp-root">
        ...
        <nav className="hp-nav">
          ...
          <div className="hp-nav-center">
            ...
            <NavLink to="/analyser" className="hp-nav-tab">⚡ Analyser</NavLink>
            {user?.role === 'admin' && (
          ...
          <div className="hp-nav-right" ref={navRef}>
            ...
            {dropdownOpen && (
              <div className="hp-dropdown">
                ...
                {/* ✅ Dashboard */}
                <NavLink to="/dashboard" className="hp-dropdown-item" onClick={() => setDropdownOpen(false)}>
        ...
        <div className="hp-hero">
          <div className="hp-hero-orb1" /><div className="hp-hero-orb2" />
          <div className="hp-hero-inner">
            <div className="hp-hero-top">
              <div>
                <div className="hp-hero-eyebrow">// Dashboard</div>
                <div className="hp-hero-title">Welcome back, <em>{user?.firstName}</em>.<br />Keep breaking curses.</div>
                <div className="hp-hero-meta">{solved} of {total} problems solved · {pct}% complete</div>
              </div>
            </div>
            <div className="hp-stats-row">
              <StatCard icon="☠" iconCls="hp-stat-icon-red"    cardCls="hp-stat-card-red"    valCls="hp-stat-val-red"    value={`${solved}/${total}`} label="Total Solved"    progress={pct} />
              <StatCard icon="✓" iconCls="hp-stat-icon-green"  cardCls="hp-stat-card-green"  valCls="hp-stat-val-green"  value={easy}                 label="Easy Problems" />
              <StatCard icon="~" iconCls="hp-stat-icon-yellow" cardCls="hp-stat-card-yellow" valCls="hp-stat-val-yellow" value={medium}               label="Medium Problems" />
              <StatCard icon="☢" iconCls="hp-stat-icon-hard"   cardCls="hp-stat-card-hard"   valCls="hp-stat-val-hard"   value={hard}                 label="Hard Problems" />
            </div>
          </div>
        </div>
        ...
        <div className="hp-main">
          <div className="hp-section-head">
            ...
            <div className="hp-diff-tabs">
              {DIFF_TABS.map(d => (
                <button key={d} className={`hp-diff-tab d-${d} ${filters.difficulty === d ? 'active' : ''}`}
          ...
          <div className="hp-filter-bar">
            ...
            {hasFilters && <button className="hp-clear-btn" onClick={clearFilters}>✕ Clear</button>}
          </div>

          {!loading && filtered.length > 0 && (
            <div className="hp-table-head">
              <span>#</span><span>Title</span><span>Difficulty</span><span>Tag</span><span>Status</span>
            </div>
          )}

          {loading ? (
            <div className="hp-loading"><div className="hp-spinner" /> Loading your problem set...</div>
          ) : filtered.length === 0 ? (
