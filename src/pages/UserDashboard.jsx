...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .db-root {
    ...
    font-family: var(--mono); cursor: auto; position: relative;
  }
  ...
  .db-nav {
    ...
    background: rgba(3,2,10,.92); border-bottom: 1px solid var(--border);
    backdrop-filter: blur(24px);
    animation: db-slideDown .6s cubic-bezier(.16,1,.3,1) both;
  }
  ...
  .db-logout-btn {
    padding: 7px 16px; border-radius: 8px; font-size: .75rem;
    background: rgba(255,45,45,.08); border: 1px solid rgba(255,45,45,.2);
    color: var(--red); cursor: pointer; font-family: var(--mono);
    transition: all .2s;
  }
  ...
  .db-hero {
    ...
    padding: 40px 36px 36px; border-bottom: 1px solid var(--border);
  }
  .db-hero::before {
    content: ''; position: absolute; inset: 0;
    ...
    background-size: 36px 36px; animation: db-grid 24s linear infinite;
  ...
  .db-avatar-big {
    ...
    background: linear-gradient(135deg, var(--red), var(--orange));
    display: flex; align-items: center; justify-content: center;
    font-family: var(--display); font-size: 2rem; font-weight: 800; color: #fff;
    box-shadow: 0 0 32px rgba(255,45,45,.3);
    animation: db-avatarIn .7s cubic-bezier(.16,1,.3,1) both;
  }
  ...
  .db-hero-joined { font-size: .68rem; color: var(--dim); margin-top: 4px; }
  ...
  .db-stat-card {
    ...
    animation: db-fadeUp .6s cubic-bezier(.16,1,.3,1) both;
  }
  ...
  .db-cal-grid { display: grid; grid-template-columns: repeat(26, 1fr); gap: 3px; margin-top: 14px; }
  .db-cal-cell {
  ...
  .db-cal-cell:hover { transform: scale(1.3); }
  ...
  .db-cal-legend-cell { width: 11px; height: 11px; border-radius: 2px; }
  ...
  .db-sub-row {
    ...
    align-items: center; padding: 12px 16px; border-radius: 9px;
    margin-bottom: 6px; background: var(--card2);
...
export default function UserDashboard() {
  ...
  const activityCells = generateActivityData(submissions);
  const recentSubs    = [...submissions].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 8);

  // current streak
  const streakDays = (() => {
    const map = {};
    submissions.forEach(s => { const d = new Date(s.createdAt).toISOString().split('T')[0]; map[d] = true; });
    let streak = 0;
    const today = new Date();
    for (let i = 0; i < 365; i++) {
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="db-sec-title" style={{ marginBottom: 16 }}>
                <span className="db-sec-title-accent">//</span> Overview
              </div>
              <div className="db-stats-grid" style={{ marginBottom: 32 }}>
                {[
                  ...
                  <div key={i} className={`db-stat-card db-stat-card-${s.cls}`} style={{ animationDelay: `${i * 0.07}s` }}>
                    ...
                    <div className="db-progress">
                      <div className="db-progress-fill" style={{ width: s.pct + '%' }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="db-two-col">
                ...
                <div className="db-calendar-card">
                  ...
                  <div className="db-cal-grid">
                    {activityCells.map((c, i) => (
                      <div key={i} className={`db-cal-cell level-${c.level}`} title={`${c.date}: ${c.count} submissions`} />
                    ))}
                  </div>
                  <div className="db-cal-legend">
                    <span>Less</span>
                    {[0,1,2,3,4].map(l => (
                      <div key={l} className={`db-cal-legend-cell db-cal-cell level-${l}`} style={{ width: 11, height: 11, aspectRatio: 'unset' }} />
                    ))}
                    <span>More</span>
                  </div>
                </div>
                ...
                <div className="db-diff-card">
                  <div className="db-sec-title">
                    <span className="db-sec-title-accent">//</span> Difficulty Breakdown
                  </div>
              ...
              <div className="db-profile-card" style={{ marginBottom: 32 }}>
                {[
                  ...
                  <>
                    ...
                    {recentSubs.map((s, i) => (
                      <NavLink key={i} to={`/problem/${s.problemId}`} className="db-sub-row">
                        <span className="db-sub-title">{s.problemName || s.problemId || 'Problem'}</span>
